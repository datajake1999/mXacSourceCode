...
class CSRPronTree : public CEscObject {
public:
   CSRPronTree (void);
   ~CSRPronTree (void);
...
class CSRHyp : public CEscObject {
public:
   CSRHyp (void);
   ~CSRHyp (void);
   CSRHyp *Clone (void);
...
// #endif
   ...
   // away from basics
   if (ppiRough)
      fScore = fScore * 0.8 + ppiRough->Compare (paSR, dwNum, fMaxSpeechWindow, fForUnitSelection, fAllowFeatureDistortion, fFast,
         fHalfExamples) * 0.2;
   return fScore;
#endif OLDSR
}
...
returns
   BOOL - TRUE if success
*/
BOOL CVoiceFile::TrainSingleWave (PCM3DWave pWave, HWND hWnd, PCProgress pProgress, BOOL fLessPickey,
                                  PCHashString phFunctionWords, BOOL fFillFunctionWords)
{
   ...
   for (i = 0; i+1 < dwNum; i++) {
      ...
      if (!_wcsicmp(szName, pszSilence)) {
         if (!fLessPickey && (i && (dwLength < pWave->m_dwSamplesPerSec / 25))) {
      ...
      // length too long?
      if (!fLessPickey && (dwLength+10 < pWave->m_dwSamplesPerSec / 50) && (m_dwTooShortWarnings < 10)) { // BUGFIX bit of rounoff err
      ...
      if (!fLessPickey && (dwLength > pWave->m_dwSamplesPerSec /*/ 2*/)) {
   ...
   for (i = 0; i+1 < dwNum; i++) {
      ...
      for (j = 0; ; j++) {
         ...
         pPhone->Train (psab + dwStart, dwEnd - dwStart, fMaxEnergy, szBefore, szAfter, m_pLex, !m_fCDPhone,
            fWeight / (fp)OVERTRAIN / (fp)dwOtherStressCount);
...
//#define HYPONTOP     (MAXHYP/5) // BUGFIX - Make /5 instead of /10
   ...
   fBest += (fp)pWave->m_dwSRSAMPLESPERSEC/2.0 /* 1/2 second */ * (-(SRNOISEFLOOR)/2);   // 1 second of really bad results
...
#define OLDHYP    (pWave->m_dwSRSAMPLESPERSEC*2)      // 1 seconds
...
BOOL CVoiceFile::HypRecognize (PWSTR pszText, PCM3DWave pWave, BOOL fHalfExamples, PCProgressSocket pProgress)
{
   ...
   for (dwPass = 0; dwPass < SRPASSES; dwPass++) {
      DWORD dwTimeAccuracy = ACCURACYPERPASS * (SRPASSES-dwPass-1) + 1;

      if (pProgress)
         pProgress->Push (sqrt((fp)dwPass / (fp)SRPASSES), sqrt((fp)(dwPass+1) / (fp)SRPASSES));
      ...
      hi.fFast = (dwPass+1 < SRPASSES) && (dwTimeAccuracy > 2);
         // BUGFIX - Was just !dwPass, but changed to (dwPass+1 < SRPASSES)
   ...
   case ESCN_BUTTONPRESS:
      {
         ...
         if (!_wcsicmp(psz, L"fortts")) {
            ...
            dwNum = RecBatch (WAVECALC_SEGMENT, pPage->m_pWindow, pVF->m_szDefTraining, FALSE, FALSE,
               &wtd, pVF->m_pWaveDir, pVF->m_szFile, L"SpeechRec");
            ...
            DWORD dwNum;
            dwNum = RecBatch (WAVECALC_SEGMENT, pPage->m_pWindow, pVF->m_szDefTraining, FALSE, FALSE,
               pVF->m_pWaveToDo, pVF->m_pWaveDir, pVF->m_szFile, L"SpeechRec");
            ...
            pVF->TrainRecordings (pPage->m_pWindow->m_hWnd, GetKeyState (VK_CONTROL) < 0);
            return TRUE;
         }
...
BOOL CVoiceFile::TrainRecordings (HWND hWnd, BOOL fLessPickey)
{
   // clear out
   TrainClear();
   ...
   m_dwTooShortWarnings = 0;
   {
      // progress bar
      CProgress Progress;
      Progress.Start (hWnd, "Analyzing recordings...", TRUE);
      ...
      for (dwPass = 0; dwPass < 2; dwPass++) {
         ...
         for (i = 0; i < dwNum; i++) {
            ...
            Progress.Update (0);
            fRet = TrainSingleWave (&Wave, hWnd, &Progress, fLessPickey, &hWordWeight, !dwPass);
            Progress.Pop ();
   ...
   if (dwAdded) {
      EscMessageBox (hWnd, ASPString(),
         ...
         (PWSTR)gMemTemp.p,
         MB_ICONEXCLAMATION | MB_OK);
#ifndef _DEBUG // on release mode fail if dont have recordings of all phonemes
      TrainClear();
      return FALSE;
#endif
   }
                              ...
                              PCListFixed plPhone)
{
   ...
   for (i = 0; i < pNode->ContentNum(); i++) {
      ...
      for (j = 0; j < dwSize; j++) {
         ...
         if (j == 0)
            dwPhone |= (1 << 24);   // note that start of word
         if (j+1 == dwSize)
            dwPhone |= (2 << 24);   // note that end of word
                                  ...
                                  fp fDbAbovePhoneCompare, fp fDbCompare, fp fDbThis)
{
   ...
   fErr += fabs(fDbAvobePhoneThis - fDbAbovePhoneCompare);
                     ...
                     PSRFEATURESMALL pSRFModel, fp fSRFModel)
{
   ...
   // convert to energy
   fp afVoicedOrigMain[SRDATAPOINTSSMALL], afNoiseOrigMain[SRDATAPOINTSSMALL], afVoicedOrigDelta[SRDATAPOINTSSMALL], afNoiseOrigDelta[SRDATAPOINTSSMALL];
   for (i = 0; i < SRDATAPOINTSSMALL; i++) {
...
#define SMALLOCTAVE     (SRDATAPOINTSSMALL / SROCTAVE)
   DWORD dwStretch;
   fp afVoicedStretchMain[SRDATAPOINTSSMALL], afNoiseStretchMain[SRDATAPOINTSSMALL], afVoicedStretchDelta[SRDATAPOINTSSMALL], afNoiseStretchDelta[SRDATAPOINTSSMALL];
                             ...
                             BOOL fHalfExamples)
{
   ...
   DWORD dwTopN = min(dwMaxCompare/(fForUnitSelection ? TOPPERCENTUNIT : TOPPERCENT)+1, (DWORD)TOPN);   //+1 makes sure round up
   ...
   dwMaxCompare = max(min(dwMaxPhoneSamples/2, dwMaxCompare), dwTopN);
