/*************************************************************************************
ResMusic.cpp - Code for the Music resource.

begun 13/8/04 by Mike Rozak.
...
// OWD - Information for OpenMusicDialog
typedef struct {
   WCHAR       szFile[256];     // Music file
   fp          fVolL;            // left vol
   fp          fVolR;            // right vol
...
ResMusicPage
*/
BOOL ResMusicPage (PCEscPage pPage, DWORD dwMessage, PVOID pParam)
{
   ...
   case ESCN_BUTTONPRESS:
      {
         ...
         if (!_wcsicmp(psz, L"open")) {
            if (!OpenMusicDialog (pPage->m_pWindow->m_hWnd, powd->szFile, sizeof(powd->szFile)/sizeof(WCHAR), FALSE))
               return TRUE;
   ...
   case ESCM_SUBSTITUTION:
      {
         ...
         if (!_wcsicmp(p->pszSubName, L"PAGETITLE")) {
            p->pszSubString = L"Music resource";
            return TRUE;
         }
      }
      break;
...
/*************************************************************************
ResMusicEdit - Modify a resource Music. Uses standard API from mifl.h, ResourceEdit.
*/
...
redo:
   psz = Window.PageDialog (ghInstance, IDR_MMLRESMUSIC, ResMusicPage, &owd);
